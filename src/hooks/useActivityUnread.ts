import { useMemo } from 'react'
import { useApp } from '@/contexts/app-context'
import { useNostrFeed } from '@/hooks/useNostrFeed'
import { ACTIVITY_KINDS, ACTIVITY_WINDOW_DAYS } from '@/lib/activity/constants'
import { useActivitySeen } from '@/hooks/useActivitySeen'
import type { FeedSection, NostrEvent } from '@/types'

export function useActivityUnread(options: { enabled?: boolean } = {}) {
  const { enabled = true } = options
  const { currentUser } = useApp()
  const { seenAt } = useActivitySeen()

  // Stable section that NEVER changes – filter is independent of seenAt
  const section = useMemo<FeedSection>(() => {
    if (!currentUser?.pubkey || !enabled) {
      return {
        id: 'activity-unread-disabled',
        label: 'Activity Unread',
        filter: { kinds: [], limit: 1 },
      }
    }

    const now = Math.floor(Date.now() / 1000)
    const windowStart = now - (ACTIVITY_WINDOW_DAYS * 24 * 60 * 60)

    return {
      id: `activity-unread:${currentUser.pubkey}`,
      label: 'Activity Unread',
      // Don't include seenAt in filter – load all activity and filter client-side
      // This prevents the subscription from resetting when seenAt changes
      filter: {
        kinds: ACTIVITY_KINDS,
        '#p': [currentUser.pubkey],
        since: windowStart,
        limit: 120,
      },
    }
  }, [currentUser?.pubkey, enabled])

  const { events, loading } = useNostrFeed({
    section,
    enabled: section.id !== 'activity-unread-disabled',
  })

  // Filter to only unread events client-side
  const unreadEvents = useMemo(() => {
    if (!currentUser?.pubkey) return []
    return events.filter((event) => (
      event.pubkey !== currentUser.pubkey && event.created_at > seenAt
    ))
  }, [currentUser?.pubkey, events, seenAt])

  const unreadCount = unreadEvents.length

  return {
    unreadCount,
    hasUnread: unreadCount > 0,
    loading,
  }
}
