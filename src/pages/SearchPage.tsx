...
export default function SearchPage() {
  ...
  const visibleEvents = useMemo(
    () => events.filter((event) => !eventModerationIds.has(event.id) || allowedEventIds.has(event.id)),
    [allowedEventIds, eventModerationIds, events],
  )
  const visibleProfiles = useMemo(
    () => profiles.filter((profile) => !profileModerationIds.has(profile.pubkey) || allowedProfileIds.has(profile.pubkey)),
    [allowedProfileIds, profileModerationIds, profiles],
  )
  ...
  const fetchLoading = localLoading || relayLoading
