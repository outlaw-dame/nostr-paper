 ...
 *  1. Continue as saved user (NIP-46 or read-only pubkey from prior session)
 *  2. Use a NIP-07 browser extension or NIP-46 remote signer
...
function KeyInputScreen({
  ...
  const handleSubmit = useCallback(async () => {
    ...
    if (!trimmed) {
      setError(mode === 'npub' ? tApp('onboardEnterPublicError') : tApp('onboardInvalidNip46Error'))
      return
    }
    setError(null)
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="relative mb-2">
        <input
          type="text"
          placeholder={isNpub ? tApp('onboardPublicPlaceholder') : tApp('onboardNip46Placeholder')}
          value={value}
      ...
      >
        {loading
          ...
          : (isNpub ? tApp('onboardBrowseReadOnly') : tApp('onboardConnectNip46'))}
      </button>
    </div>
  )
}
...
export default function OnboardPage() {
  ...
  const [inputMode, setInputMode] = useState<'npub' | 'nip46' | null>(null)
  ...
  const [savedMethod, setSavedMethod] = useState<'npub' | 'nip46' | null>(null)
  ...
  useEffect(() => {
    ...
    const bunker = sessionStorage.getItem(STORAGE_KEY_NIP46_BUNKER)
    ...
    <div className="min-h-dvh bg-[rgb(var(--color-bg))] flex flex-col px-6 pt-safe-top pb-safe-bottom">
      <div className="flex-1 flex flex-col pt-16 pb-8 max-w-sm mx-auto w-full">
        ...
        >
          {extLoading
            ? tApp('onboardConnecting')
            : (hasExtension ? tApp('onboardUseExtension') : tApp('onboardUseNip46'))}
        </button>

        {/* Remote signer fallback */}
        {hasExtension && (
          <button
            type="button"
