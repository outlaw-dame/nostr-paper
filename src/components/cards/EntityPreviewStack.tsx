...
function PrimaryUrlSlot({
  ...
  const isEmbeddedVideo = Boolean(youtubeId || vimeoId || peertubeEmbed)
  const feedLike = React.useMemo(
    () => !isEmbeddedVideo && looksLikeFeedUrl(candidate.url),
    [candidate.url, isEmbeddedVideo],
  )
  const { data, loading } = useLinkPreview(candidate.url, { enabled: !isEmbeddedVideo && !feedLike })
