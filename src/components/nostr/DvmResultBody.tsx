...
export function DvmResultBody({
  ...
  const { event: fetchedRequest, loading: requestLoading } = useEvent(
    result?.requestEvent ? null : result?.requestEventId,
  )
  ...
  const requestEvent = result.requestEvent ?? fetchedRequest
  const canDecrypt = result.hasEncryptedPayload && canDecryptDvmEvent(event, currentUser?.pubkey)
  ...
  const handleDecrypt = async () => {
    ...
    try {
      ...
      setDecryptError(loadError instanceof Error ? loadError.message : 'Failed to decrypt DVM result.')
    } finally {
      setDecrypting(false)
    }
  }

  const visibleContent = decryptedContent ?? (!result.isEncrypted ? result.content : '')
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-2 text-[14px] leading-6 text-[rgb(var(--color-label-secondary))]">
            Result for request kind {result.requestKind}.
          </p>
        </div>
      ...
      {result.amount && (
        <div className="rounded-[14px] border border-[rgb(var(--color-fill)/0.10)] bg-[rgb(var(--color-bg))] p-3">
          ...
          <p className="mt-1 text-[14px] text-[rgb(var(--color-label-secondary))]">
            {formatMsats(result.amount.msats)}
          </p>
          {result.amount.invoice && (
      ...
      {result.hasEncryptedPayload && (
        <div className="space-y-3 rounded-[16px] border border-[rgb(var(--color-fill)/0.10)] bg-[rgb(var(--color-bg))] p-3">
          <p className="text-[14px] leading-6 text-[rgb(var(--color-label-secondary))]">
            This result content is encrypted for the customer and service provider.
          </p>
          ...
          {!canDecrypt && (
            <p className="text-[13px] leading-6 text-[rgb(var(--color-label-tertiary))]">
              Connect the participating signer with NIP-04 support to decrypt this result.
            </p>
          )}
